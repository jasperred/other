(function($) {
	...
	$.mcDropDownMenu = function(el, list, options) {
				...
				.bind(
						...
						function(e) {
							...
							if ($ul.length) {
								...
								if (bIsParent && $keylist
										&& $ul[0] === $keylist[0]) {
									clearTimeout(iBlurTimeout);
									updateValue(
											$target.find("> ul > li:first"),
											false);
									e.stopPropagation();
									return false
								} else {
									if (!settings.allowParentSelect
											&& bIsParent) {
										return false
									}
								}
								...
								if (settings.click != null
										&& settings.click.apply($target, [ e,
												thismenu, settings ]) == false) {
									return false
								}
							}
							thismenu.closeMenu()
		...
		$divInput = $(
				'<div class="mcdropdown"><a href="#" tabindex="-1"></a><input type="hidden" name="'
						+ (el.name || el.id)
						+ '" id="'
						+ (el.id || el.name)
						+ '" /></div>').appendTo(
				$('<div style="position: relative;"></div>')).parent();
		...
		$self.width(di.width).filter(":input").attr("autocomplete", "off")
				.bind("keypress", checkKeypress).bind("mousedown", function(e) {
		...
		this.disable = function(status) {
			bDisabled = !!status;
			$divInput[bDisabled ? "addClass" : "removeClass"]
					("mcdropdownDisabled");
		...
		function addBindings(el) {
			removeBindings(el);
			$("> li", el).bind("mouseover", hoverOver).bind("mouseout",
					hoverOut)
		}
		function removeBindings(el) {
			$("> li", el).unbind("mouseover", hoverOver).unbind("mouseout",
					hoverOut)
		}
		...
		function getShadow(depth) {
			...
			if (!shadows[depth]) {
				shadows[depth] = $('<div class="mcdropdown_shadow"></div>')
						.appendTo("body");
				if (!!$.fn.bgIframe) {
		...
		function showBranch() {
			...
			$(this).parent().find("> li ul:visible").not($ul).parent().each(
					function() {
						hideBranch.apply(this)
					});
			columnizeList($ul);
			...
			if (menuBottom > sd.y) {
				$ul.css("top", li_coords.bottom - (menuBottom - sd.y)
						- settings.screenPadding)
			}
		...
		function hideBranch() {
			...
			if (isIE6) {
				$ul.css("visibility", "hidden").parent()
		...
		function position($el, bUseOffset) {
			...
			var pos = $.extend($el[bUseOffset === true ? "offset" : "position"]
					(),
					{
						...
						marginLeft : parseInt($.curCSS($el[0], "marginLeft",
								true), 10) || 0,
						marginRight : parseInt($.curCSS($el[0], "marginRight",
								true), 10) || 0,
						marginTop : parseInt($
								.curCSS($el[0], "marginTop", true), 10) || 0,
						marginBottom : parseInt($.curCSS($el[0],
								"marginBottom", true), 10) || 0
					});
			if (pos.marginTop < 0) {
		...
		function getPadding(el, name) {
			var torl = name == "height" ? "Top" : "Left", borr = name == "height" ? "Bottom"
					: "Right";
			return (parseInt("0"
					+ $.curCSS(el, "border" + torl + "Width", true), 10)
					+ parseInt("0"
							+ $.curCSS(el, "border" + borr + "Width", true), 10)
		...
		function getListDimensions($el, cols) {
			if (!$el.data("dimensions")) {
				...
				var width = (($el === $list) && ($el.data("width") * cols < ddWidth)) ? Math
						.floor(ddWidth / cols)
						: $el.data("width");
				$el.data("dimensions",
						{
		...
		function columnizeList($el) {
			...
			var cols = !!arguments[1] ? arguments[1]
					: (items <= settings.minRows) ? 1
							: (calculatedCols > settings.targetColumnSize) ? calculatedCols
									: settings.targetColumnSize;
			...
			$children
					.each(function(i) {
						var currentItem = i + 1;
						var nextItemColumn = Math.floor((currentItem / items)
								* cols);
						...
						$li.css( {
							...
							marginTop : marginTop,
							width : widths.item
						})[((nextItemColumn > column) || (currentItem == items)) ? "addClass"
								: "removeClass"]("mc_endcol")[(marginTop != 0) ? "addClass"
								: "removeClass"]("mc_firstrow");
			...
			if (($el !== $list)
					&& (maxColumnHeight + (settings.screenPadding * 2) >= getScreenDimensions().height)) {
		...
		function onFocus(e) {
			...
			typedText = treePath.join(settings.delim)
					+ (treePath.length > 0 ? settings.delim : "");
			setTimeout(function() {
				setSelection($self[0], lastDelim, lastDelim
						+ currentNode.length)
			}, 0);
			if (!$keylist) {
				$keylist = $('<ul class="mcdropdown_autocomplete"></ul>')
						.appendTo("body");
				if (isIE6 && !!$.fn.bgIframe) {
					$keylistiframe = $("<div></div>").bgIframe().appendTo(
							"body")
				}
			}
			var hideResults = !(settings.showACOnEmptyFocus && (typedText.length == 0));
			var $siblings = ($current.length == 0 || $current
					.hasClass("mc_root")) ? $list.find("> li") : $current
					.parent().find("> li");
		...
		function onBlur(e) {
			iBlurTimeout = setTimeout(function() {
				...
				if (!settings.allowParentSelect && $current.is(".mc_parent")) {
					var value = $current.find("li:not('.mc_parent'):first")
							.attr(settings.valueAttr);
		...
		function showMatches($li, hideResults) {
			var bCached = ($li === oldCache), $items = bCached ? $keylist.find(
					"> li").removeClass("mc_hover mc_hover_parent mc_firstrow")
					: $li.clone().removeAttr("style").removeClass(
							"mc_hover mc_hover_parent mc_firstrow mc_endcol")
							.filter(":last").addClass("mc_endcol").end();
			if (!bCached || $keylist.is(":hidden")) {
				$keylist.empty().append($items).width(
						$divInput.outerWidth()
								- getPadding($keylist[0], "width")).css(
						"height", "auto");
				anchorTo($divInput.parent(), $keylist, true);
				$items.hover(function() {
					$keylist.find("> li").removeClass(
							"mc_hover_parent mc_hover");
					$(this).addClass("mc_hover")
				}, function() {
					$(this).removeClass("mc_hover")
				});
				$items.find("> ul").css("display", "none");
				$keylist.show().css("visibility",
						(hideResults === true) ? "hidden" : "visible");
				if (isIE6) {
			...
			var $current = $keylist.find("li[" + settings.valueAttr + "='"
					+ $hidden.val() + "']");
			$current.addClass("mc_hover"
					+ ($current.is(".mc_parent") ? "_parent" : ""));
		...
		function checkKeypress(e) {
			var key = String.fromCharCode(e.keyCode || e.charCode)
					.toLowerCase();
			var $current = getCurrentListItem();
			var $lis = ($current.length == 0 || $current.hasClass("mc_root")) ? $list
					.find("> li")
					: $current.parent().find("> li");
				...
				if (e.keyCode == 40) {
					...
					if (e.keyCode == 27) {
						...
						if (e.keyCode == 8 || e.keyCode == 37) {
							compare = (e.keyCode == 37) ? "" : currentNode
									.substring(0, currentNode.length - 1);
							...
							if (e.keyCode == 9 || e.keyCode == 13
									|| e.keyCode == 39 || key == settings.delim) {
			...
			if (matchesCache.length > 0) {
				typedText = treePath.join(settings.delim)
						+ (treePath.length > 0 ? settings.delim : "") + compare;
				updateValue(matchesCache.eq(0), true)
			} else {
				compare = compare.length ? compare.substring(0,
						compare.length - 1) : "";
				matchesCache = findMatches($lis, compare);
				if (matchesCache.length > 0) {
		...
		function updateValue($li, keepTypedText) {
			var $siblings = keepTypedText ? matchesCache
					: ($li.length == 0 || $li.hasClass("mc_root")) ? $list
							.find("> li") : $li.parent().find("> li");
			var treePath = getTreePath($li);
			var currentNode = treePath.pop().toLowerCase();
			if (!keepTypedText) {
				typedText = treePath.join(settings.delim).toLowerCase()
						+ (treePath.length > 0 ? settings.delim : "")
			}
			thismenu.setValue($li.attr(settings.valueAttr), true);
			setSelection($self[0], typedText.length, currentNode.length
					+ typedText.length);
			$siblings.filter(".mc_hover,.mc_hover_parent").removeClass(
					"mc_hover mc_hover_parent");
		...
		function getSelection(field) {
			...
			if (field.setSelectionRange) {
				text = field.value.substring(field.selectionStart,
						field.selectionEnd)
			} else {
		...
		function scrollIntoView($el, center) {
			...
			if ((el.offsetTop + el.offsetHeight) > (scrollable.scrollTop + scrollable.clientHeight)) {
				scrollable.scrollTop = $el.offset().top
						+ (scrollable.scrollTop - $keylist.offset().top)
						- ((scrollable.clientHeight / ((center == true) ? 2 : 1)) - ($el
								.outerHeight() + s.pBottom))
			} else {
				if (el.offsetTop - s.bTop - s.bBottom <= (scrollable.scrollTop
						+ s.pTop + s.pBottom)) {
					scrollable.scrollTop = $el.offset().top
							+ (scrollable.scrollTop - $keylist.offset().top)
							- s.pTop
