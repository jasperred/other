...
public class ServiceOrderFinishAction {
...
private ServiceOrderServices serviceOrderServices;
	...
	public String finishServiceOrder()
	{
		try {
//			if(WebUtil.isNull(serviceFinishDate))
//			{
//				message = "维修完成时间未填写";
//				return "error";
//			}
			if(WebUtil.isNull(customerRetrieveDate))
			{
				message = "客户取回时间未填写";
				return "error";
			}
			if(WebUtil.isNull(fixPeople))
			{
				message = "维修人未填写";
				return "error";
			}
			...
			param.put("ServiceFinishDate", WebUtil.filterSpecialCharacters(serviceFinishDate));
			param.put("FixPeople", WebUtil.filterSpecialCharacters(fixPeople));
			param.put("CustomerRetrieveDate", WebUtil.filterSpecialCharacters(customerRetrieveDate));
			Map<String,String> r = serviceOrderServices.finishServiceOrder(param);
