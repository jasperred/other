...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<div id="search-box">
			...
			<table width=100% id="delivery-info" rules="none" border="1">
				<tr>
					<td class="">
				...
				<tr>
					<td class="">
						<input type="text" class="pure-text" value="物流发货单号"
							readonly="readonly" />
						<input type="text" id="shipping-no" onchange="checkValueLength($(this),30)" />
						<font color="red">*</font>
					</td>
					<td class="">
						<input type="text" class="pure-text" value="发货仓库"
							readonly="readonly" />
						<input type="text" id="delivery-stock" onchange="checkValueLength($(this),100)" />
						<font color="red">*</font>
					</td>
					<td class="">
						<input type="text" class="pure-text" value="发货日期"
							readonly="readonly" />
						<input type="text" id="delivery-date" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="Wdate"  />
						<font color="red">*</font>
					</td>
				</tr>
				<tr id="rate-tr1">
			...
			<table width=100% id="delivery-item" bdelivery="0" cellpadding="1"
				...
				<tbody>
					...
					<tr>
						<s:if test="status.equals(\"DELIVERY\")">
						...
						<s:else>
						<td>
						<input key="ServiceDeliveryItemId" type="hidden" value="<s:property value="id"/>"/>
						<select key="ItemChange" >
						<s:if test="ItemChange!=null&&ItemChange.equals(\"Y\")">
						<option value="Y" selected="selected">是</option><option value="N">否</option>
						</s:if>
						<s:else>
						<option value="Y">是</option><option value="N" selected="selected">否</option>
						</s:else>
						
						</select>
						</td>
						<td><input key="NewItemNo" onchange="checkValueLength($(this),30)" value="<s:property value="NewItemNo"/>"/></td>
						<td>
						<select key="HasInv" >
						<s:if test="HasInv!=null&&HasInv.equals(\"Y\")">
						<option value="Y" selected="selected">是</option><option value="N">否</option>
						</s:if>
						<s:else>
						<option value="Y">是</option><option value="N" selected="selected">否</option>
						...
						<td><input key="ItemOrderNo" onchange="checkValueLength($(this),30)" value="<s:property value="ItemOrderNo"/>"/></td>
						<td><input key="ItemOrderDate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="Wdate"  value="<s:property value="ItemOrderDate"/>"/></td>
						<td><input key="ItemDeliveryDate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="Wdate"  value="<s:property value="ItemDeliveryDate"/>"/></td>
						<td><input key="ItemArrivalDate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="Wdate"  value="<s:property value="ItemArrivalDate"/>"/></td>
						</s:else>
			...
			<form id="file-upload-form" action="serviceDeliveryFileUploadByCspcAction.action" method="post"
				enctype="multipart/form-data">
			<table width=100% id="delivery-file-upload">
					...
					<tr>
						...
						<td class="">
							...
							<input type="hidden" name="serviceDeliveryId"
								value="<s:property value="serviceDeliveryId"/>" />
						...
						<td>
							<input type="button" id="file-upload-button" value="上传" onclick="deliveryFileUpload()" />
						</td>
			...
			<table align="right"><tr><td colspan="3"><input id="wait-delivery-button" type="button" value="缺货等待" /><input id="delivery-button" type="button" value="处理完成" /><input id="delivery-back-apply-button" type="button" value="退回重新审核" /><input type="button" id="back-button" value="返回" /></td></tr></table>
		<div id="dialog-form-backMemo" title="退回说明">		
			<form>
			<table width="100%">
				<tr><td>请填写退回说明:</td></tr>
				<tr></tr>
			...
			<tr>
				<td>
					<textarea rows="3" cols="35" id="back-memo"></textarea>
				</td>
				</tr>
			</table>
			</form>
	...
	$(document).ready(function() {
		...
		$("#delivery-info-banner").bind('click', function() {
		hideAndShow('delivery-file');
		hideAndShow('file-upload-form');
	});
			...
			pageControlByCspc(unicodeToString( '<s:property value="status"/>'));
		});
</script>
</html>