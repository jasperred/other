			...
			fnUpdate : function(g, m) {
				if (g.aanFeatures.p) {
					var r = n.oPagination.iFullNumbersShowPages, s = Math
							...
							.ceil(g._iDisplayStart / g._iDisplayLength) + 1, F = "", x, u = g.oClasses;
					if (w < r) {
						s = 1;
						x = w
					} else if (y <= s) {
					...
					for (r = s; r <= x; r++)
						F += y != r ? '<span class="' + u.sPageButton + '">'
								+ r + "</span>" : '<span class="'
								+ u.sPageButtonActive + '">' + r + "</span>";
					x = g.aanFeatures.p;
					var z, U = function() {
					...
					for (s = x.length; r < s; r++)
						if (x[r].childNodes.length !== 0) {
							z = j("span:eq(2)", x[r]);
							z.html(F);
							j("span", z).bind("click.DT", U).bind(
									"mousedown.DT", C)
									.bind("selectstart.DT", C);
	...
	j.fn.dataTable = function(g) {
			...
			if (a.bInitialised === false)
				...
				for (c = a.aoColumns.length; b < c; b++)
					if (a.aoColumns[b].sWidth !== null)
						a.aoColumns[b].nTh.style.width = v(a.aoColumns[b].sWidth);
			...
			if (a.oFeatures.bSort)
				for (b = 0; b < a.aoColumns.length; b++)
					if (a.aoColumns[b].bSortable !== false) {
						...
						j(a.aoColumns[b].nTh).bind("mousedown.DT", d)
					} else
						j(a.aoColumns[b].nTh)
		...
		function Ca(a) {
			...
			if (a.oScroll.sX === "") {
				...
				if (j.browser.msie && j.browser.version <= 7)
					a.nTable.style.width = v(j(a.nTable).outerWidth()
							- a.oScroll.iBarWidth)
			} else if (a.oScroll.sXInner !== "")
				a.nTable.style.width = v(a.oScroll.sXInner);
			...
			if (a.oScroll.sY === "")
				if (j.browser.msie && j.browser.version <= 7)
					d.style.height = v(a.nTable.offsetHeight
							+ a.oScroll.iBarWidth);
			if (a.oScroll.sY !== "" && a.oScroll.bCollapse) {
				...
				if (a.nTable.offsetHeight < d.offsetHeight)
					d.style.height = v(j(a.nTable).height() + h)
			}
		...
		function P(a, b, c) {
			...
			for (b = 0; b < a.aoPreSearchCols.length; b++)
				Ea(a, a.aoPreSearchCols[b].sSearch, b,
		...
		function $(a) {
			...
			if (f == i.length && b === 0 && d == f && a.oScroll.sX === ""
				...
				if (a.oScroll.sX !== "" && a.oScroll.sXInner !== "")
					b.style.width = v(a.oScroll.sXInner);
				else if (a.oScroll.sX !== "") {
					b.style.width = "";
					if (j(b).width() < e.offsetWidth)
						b.style.width = v(e.offsetWidth)
		...
		function Ja(a, b) {
			if (a.oScroll.sX === "" && a.oScroll.sY !== "") {
				...
				b.style.width = v(j(b).outerWidth())
		}
		function Ia(a, b, c) {
		...
		function na(a) {
			if (!(!a.oFeatures.bStateSave || typeof a.bDestroying != "undefined")) {
				...
				f += "}";
				La(a.sCookiePrefix + a.sInstance, f, a.iCookieDuration,
		...
		this.fnGetNodes = function(a) {
			var b = A(this[n.iApiIndex]);
			if (typeof a != "undefined")
				return (aRowData = b.aoData[a]) ? aRowData.nTr : null;
			return R(b)
		};
		...
		this.fnUpdate = function(a, b, c, d, f) {
			...
			if (typeof a != "object") {
				...
				i = M(e, c);
				if (i !== null)
					e.aoData[b].nTr.getElementsByTagName("td")[i].innerHTML = h;
				else
					e.aoData[b]._anHidden[c].innerHTML = h
			} else {
